...
class Ignore:
    ...
    @property
    # pylint: disable-next=missing-function-docstring self explanatory function
    def keys(self) -> tuple[str, str, str, str]:
        return ("File", "Line section", "Pylint issue", "Reason")

    @property
    def values(  # pylint: disable=missing-function-docstring
        self,
    ) -> dict[str, str]:
        ...
        try:
            ...
            if rematch := PYLINT_IGNORE_PATTERN.match(line):
                ...
                ignore = Ignore(
                    ...
                    line_number,
                    rematch.group(1),
                    [reason.strip()] if (reason := rematch.group(2)) else [],
                )
...
def main():
    ...
    if export_dir and not export_dir.is_dir():
        logger.error("ERROR: Specified output directory points to a file.")
        sys.exit(1)
